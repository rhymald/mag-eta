...
func Lifecycle_Regenerate(st *State) {
	isNPC := false //(*st).Current.IsNPC()
	if !isNPC { for {
		// (*st).Current.Lock()
		// if len((*st.Current).Pool) >= functions.FloorRound((*st.Current.Atts).Capacity) { (*st).Current.Unlock() ; functions.Wait(4236) ; break }
		// (*st).Current.Unlock()
...
func Lifecycle_EffectConsumer(st *State) {
